...
@implementation IJKSDLGLView {
    ...
    SDL_VoutOverlay *_overlay; //为截图功能而添加的新成员
}
...
- (BOOL)saveVideoSnapshotAt:(NSString *)pathName
{
    if (!_overlay) {
        return NO;
    }
    AVFrame *frame =  av_frame_alloc();
    av_frame_ref(frame, _overlay->frame);
    struct SwsContext *swsContext = NULL;
    swsContext = sws_getCachedContext(swsContext, frame->width, frame->height, frame->format, frame->width,
    ...
    AVPicture picture;
    if (avpicture_alloc(&picture, AV_PIX_FMT_RGB24,  frame->width, frame->height)) {
        av_frame_unref(frame);
        av_frame_free(&frame);
    ...
    NSData *imageData = [[NSData alloc] initWithBytes:picture.data[0] length:(picture.linesize[0]*frame->height)];
    
    CGBitmapInfo bitmapInfo = kCGBitmapByteOrderDefault;
    CFDataRef data = CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, (const UInt8*)[imageData bytes],
                                                 [imageData length], kCFAllocatorNull);
    CGDataProviderRef provider = CGDataProviderCreateWithCFData(data);
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    CGImageRef cgImage = CGImageCreate(frame->width, frame->height, 8, 24, frame->width*3, colorSpace,
                                       bitmapInfo, provider, NULL, NO, kCGRenderingIntentDefault);
    UIImage* image = [UIImage imageWithCGImage:cgImage];
    NSData *pngData = UIImagePNGRepresentation(image);
    [pngData writeToFile:pathName atomically:YES];
